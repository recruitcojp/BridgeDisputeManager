...
describe("BridgeDisputeManager", function (/* accounts */) {
  ...
  it("verifyBlockHeader", async function () {
    ...
    const result = await bridgeDisputeManager.verifyBlockHeader(testBlockHeader.blockHash, testBlockHeader.blockHeader);
  ...
  it("verifyProof", async function () {
    ...
    const result = await bridgeDisputeManager.verifyProof(testProof.txHash, testProof.proof, testProof.txRoot, testProof.path);
    assert.isTrue(result);
  ...
  it("verifyProof wrong root", async function () {
    ...
    await expect(
      bridgeDisputeManager.verifyProof(testProof.txHash, testProof.proof, testProof.txRoot, testProof.path)
  ...
  it("verifyTxSignature", async function () {
    ...
    const result = await bridgeDisputeManager.verifyTxSignature(testTx.address, testTx.rawTx);

    assert.isTrue(result);
    const testTx2 = testData.getTxData(1);
    const result2 = await bridgeDisputeManager.verifyTxSignature(testTx2.address, testTx2.rawTx);
  ...
  it("verifyRawTx", async function () {
    ...
    const result = await bridgeDisputeManager.verifyRawTx(testTx.transaction, testTx.rawTx);
    assert.isTrue(result);
